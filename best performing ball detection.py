# -*- coding: utf-8 -*-
"""
Created on Sun Jan 16 23:36:50 2022

@author: shali

https://github.com/hemkum/ball-tracking-with-opencv/blob/master/ball_tracking.py
"""
...
greenLower = np.array([0,0,0], dtype=np.uint8)
greenUpper = np.array([255,255,255], dtype=np.uint8)
#greenUpper = np.array([0,0,255], dtype=np.uint8)
...
if not args.get("video", False):
	camera = cv2.VideoCapture('C:\\Users\\shali\\Documents\\Shalini Masters\\esports thesis\\sample video.mp4')
    #camera = cv2.VideoCapture( 'C:\\Users\\shali\\Documents\\Shalini Masters\\Thesis code\\code\\esports-thesis-assignment\\cutvideo.mp4')
    

# otherwise, grab a reference to the video file
else:
	camera = cv2.VideoCapture(args["video"])
#camera = camera.set(cv2.CAP_PROP_FPS,10)
    # Find OpenCV version
    #Find frame/sec
...
if int(major_ver)  < 3 :
    ...
    print ("Frames per second using video.get(cv2.CAP_PROP_FPS) : {0}".format(fps))# 30fps
